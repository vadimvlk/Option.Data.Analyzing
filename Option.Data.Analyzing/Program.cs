...
static List<OptionData> ParseMoexOptionData(string filePath)
{
    ...
    for (int i = 1; i < lines.Length; i++)
    {
        ...
        // Парсим страйк
        if (!double.TryParse(parts[strikeIndex].Replace(',', '.'), NumberStyles.Number, CultureInfo.InvariantCulture,
                out double strike))
        {
            continue;
        }
...
static double ParseDoubleWithSemicolon(string value)
{
...
static void CalculateCallPutRatio(List<OptionData> data)
{
...
static void CalculateMaxPain(List<OptionData> data, double currentPrice)
{
...
static void AnalyzeOpenInterest(List<OptionData> data)
{
...
static void CalculateProfitLoss(List<OptionData> data, double currentPrice)
{
...
static void AnalyzeCentersOfGravity(List<OptionData> data)
{
...
static void AnalyzePriceMovementPotential(List<OptionData> data, double currentPrice)
{
